...
void cpg_update_max_effort(cpg_float val){
  cpg_params_vec[0] = val;
  Canon_Outdated.h = 1;
}

void cpg_update_thrust_mat(cpg_int idx, cpg_float val){
  cpg_params_vec[idx+1] = val;
  Canon_Outdated.A = 1;
}

void cpg_update_moment(cpg_int idx, cpg_float val){
...
void cpg_retrieve_prim(){
  CPG_Prim.activation[0] = ecos_workspace->x[1];
  CPG_Prim.activation[1] = ecos_workspace->x[2];
  CPG_Prim.activation[2] = ecos_workspace->x[3];
  CPG_Prim.activation[3] = ecos_workspace->x[4];
}

// Retrieve dual solution in terms of user-defined constraints
void cpg_retrieve_dual(){
  CPG_Dual.d0[0] = ecos_workspace->y[8];
  CPG_Dual.d0[1] = ecos_workspace->y[9];
  CPG_Dual.d0[2] = ecos_workspace->y[10];
  CPG_Dual.d0[3] = ecos_workspace->y[11];
  CPG_Dual.d1[0] = ecos_workspace->z[12];
  CPG_Dual.d1[1] = ecos_workspace->z[13];
  CPG_Dual.d1[2] = ecos_workspace->z[14];
  CPG_Dual.d1[3] = ecos_workspace->z[15];
  CPG_Dual.d2[0] = ecos_workspace->z[0];
  CPG_Dual.d2[1] = ecos_workspace->z[1];
  CPG_Dual.d2[2] = ecos_workspace->z[2];
}
...
void cpg_solve(){
  ...
  if (!ecos_workspace  ) {
    ...
    if ( Canon_Outdated.A||Canon_Outdated.b||Canon_Outdated.G) {
      ...
      if ( Canon_Outdated.h) {
        for (i=0; i<22; i++) { ecos_updateDataEntry_h(ecos_workspace, i, Canon_Params_conditioning.h[i]); };
      }
    }
  }
