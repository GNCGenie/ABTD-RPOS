...
class PIDController {
    public:
        ...
        double update(double current_value, double target_value, double dt) {
            ...
            // Calculate derivative term (limit derivative term to avoid wind-up)
...
int main() {
    // Sample usage
    PIDController controller(0.1, 0.5, 0.01); // Adjust gains as needed

    double current_value = 5.0;
    double target_value = 10.0;
    double dt = 0.1;

    for (int i = 0; i < std::pow(10, 3); ++i) {
        ...
        std::cout << "Error: " << target_value - current_value << std::endl;
        current_value += control_output*dt;

        // Update target value
        target_value += 1e0/(i+1);
    }
